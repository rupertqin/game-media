...
module.exports = app => {
  class Main extends app.Service {
    async record(promotelink_id, media_id) {
      ...
      await app.redis.hset(`trace:common:${promoteLink.app_id}`, `${model}@${ip}`, `${promotelink_id}@${media_id}`)

      // increase view count
      await app.redis.hincrby('enjoy_view_count', promotelink_id, 1);
      // 如果有 udid
      if (this.ctx.query.udid) {
        await app.redis.hset(`trace:udid:${promoteLink.app_id}`, `${this.ctx.query.udid}`, `${promotelink_id}@${media_id}`)
      }
