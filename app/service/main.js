  ...
  class Main extends app.Service {
    ...
    async promote(query) {
      ...
      if (promotelinks.length > 0) {
        ...
        games = await this.app.mysql.query('SELECT * FROM pay_client_app WHERE id IN (?) ORDER BY field(id,?) LIMIT ? OFFSET ?',
          [ promotelinksIDs, promotelinksIDs, limit, limit * (page - 1) ]);
        for (let i = 0; i < games.length; i++) {
          games[i].hashid = this.ctx.helper.hashids_encode(promotelinks[i].id);
          // games[i].media_id = promotelinks[i].media_id
        }
      }
      const paginator = this.ctx.helper.paginator(limit, promotelinks.length, page)
      return { games, paginator }
    }
